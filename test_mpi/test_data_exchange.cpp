...
int main(int argc, char** argv) {
    ...
    double orig_psnr = -1;
    if (1) {
        orig_psnr = get_psnr_mpi(orig_copy.data(), data.get(), num_elements, cart_comm);
        if (mpi_rank == 0) {
    ...
    for (int i = 0; i < 3; i++) {
        if (coords[i] == dims[i] - 1 || coords[i] == 0) {
            w_block_dims[i] = block_dims[i] + 1 * extend_size;
        } else {
            w_block_dims[i] = block_dims[i] + 2 * extend_size;
        }
    ...
    if (1)
        data_exhange3d_extended(quant_inds.data(), block_dims, block_strides, w_quant_inds.data(), w_block_dims,
                                w_block_strides, extend_size, coords, dims, cart_comm);
    // barrier
    ...
    {
        char out_filename[100];
        sprintf(out_filename, "%s/%s_%d_%d_%d-%d-%d-%d.wquant.i32", out_dir.c_str(), name_prefix.c_str(), coords[0], coords[1],
                coords[2],w_block_dims[0], w_block_dims[1], w_block_dims[2]);
        writefile<int>(out_filename, w_quant_inds.data(), w_quant_inds.size());
    }
    ...
    if (0) {  // compensation
        if (operation && !use_rbf) {
            compensation_idw(compensation_map.data(), data.get(), distance.data(), distance_neutral.data(),
                             sign_map.data(), block_size, compensation_magnitude);
        }

        if (operation && use_rbf) {
            compensation_rbf(compensation_map.data(), data.get(), distance.data(), index.data(),
                             distance_neutral.data(), index_neutral.data(), orig_dims, sign_map.data(), block_size,
    ...
    double psnr = -1;
    if (0) {
        psnr = get_psnr_mpi(orig_copy.data(), data.get(), num_elements, cart_comm);
        if (mpi_rank == 0) {
            printf("PSNR: %f\n", psnr);
        }
    }
