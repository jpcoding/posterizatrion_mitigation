...
class CompensationIDW {
   public:
            ...
            if (boundary_map[i] == 1)  // non-boundary points ·
            {
                ...
                for (size_t j = 0; j < input_size; j++) {
                    if (boundary_map[j] == 0) {
                        int xx = j / dims[1];
                        int yy = j % dims[1];
                        double distance = std::sqrt((x - xx) * (x - xx) + (y - yy) * (y - yy));
                        double w = 1.0 / (distance);
                        sum_wu += w * compensation_map[j];
                        sum_w += w;
                    }
                }
                compensation_map[i] = sum_wu / sum_w;
            }
        }
    ...
    std::vector<T_data> get_compensation_map_3d() {
        ...
        std::vector<std::tuple<int, int, int>> coords(input_size);
#pragma omp parallel for
        for (size_t i = 0; i < input_size; i++) {
            coords[i] = {i / (dims[1] * dims[2]), (i / dims[2]) % dims[1], i % dims[2]};
        }

// Second pass: Compute non-boundary compensation using precomputed values
#pragma omp parallel for schedule(dynamic)
        for (size_t i = 0; i < input_size; i++) {
            if (boundary_map[i] == 1 ) {  // Non-boundary points
                auto [x, y, z] = coords[i];
                double sum_wu = 0, sum_w = 0;

                for (size_t j = 0; j < input_size; j++) {
                    if (boundary_map[j] == 0 ) {
                        auto [xx, yy, zz] = coords[j];
                        double distance = std::sqrt((x - xx) * (x - xx) + (y - yy) * (y - yy) + (z - zz) * (z - zz));
                        if (distance > 0) {  // Avoid self-contribution
                            double w = 1.0 / distance;
                            sum_wu += w * compensation_map[j];
                            sum_w += w;
                        }
                ...
                compensation_map[i] = (sum_w > 0) ? sum_wu / sum_w : 0;
            }
        }
