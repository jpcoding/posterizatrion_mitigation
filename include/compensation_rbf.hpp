#ifndef COMPENSATION_RBF_HPP
#define COMPENSATION_RBF_HPP
...
class CompensationRBF {
   public:
    CompensationRBF(int n, int *dims, T_data *dec_data, T_quant *quant, double compensation_value) {
        N = n;
    ...
    std::vector<T_data> get_compensation_map_2d() {
        ...
        // writefile("edge_compensation.f32", compensation_map.data(), input_size);
        timer.start();
        ...
        alglib::rbfsetalgohierarchical(model, 6, 1, 0.0);
        alglib::rbfbuildmodel(model, rep);
        // for non edge points, use rbf to get the values
        std::cout << "rbf model building time = " << timer.stop() << std::endl;
        timer.start();
        for(size_t i = 0; i < input_size; ++i)
        {
            if(boundary_map[i] == 0)
            {
                int x = i / dims[1];
                int y = i % dims[1];
                compensation_map[i] = alglib::rbfcalc2(model,(double) x, (double)y);
            }
        }
        std::cout << "rbf eval time = " << timer.stop() << std::endl;
    ...
    std::vector<T_data> get_compensation_map_3d() {
        ...
        // use rbf to get the compensation map
        alglib::rbfmodel model; 
        ...
        for(size_t i = 0; i < input_size; ++i)
        {
            if(boundary_map[i] == 0)
            {
                ...
                int y = (i / dims[2]) % dims[1];
                int z = i % dims[2];
                compensation_map[i] = alglib::rbfcalc3(model,(double) x, (double)y, (double)z);
            }
        }
        return compensation_map;
    }
