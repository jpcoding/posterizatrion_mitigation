...
test="../../../install/bin/test_mpi/test_embarrassingly_parallel"
data_dir={data_dir}
out_dir={out_dir}
mkdir -p $out_dir 
iters=5
...
mpirun -n {mpi_nps} $test --mpidims {" ".join(str(x) for x in mpi_dims )}  -m rel -e $eb --dir $data_dir  --outdir $out_dir --prefix vx --origdims 4096 4096 4096 --use_rbf 0 --local_edt 1 --local_quant 1
done
done 
...
eb_list = [ 0.001]
job_names = ["em_8x8x8", "em_8x8x4", "em_8x4x4", "em_4x4x4"] 
input_dir_names = ["vx_8x8x8", "vx_8x8x4", "vx_8x4x4", "vx_4x4x4"]
data_dir_root = "????/data/isotropic/" 
out_dir_root = "???/data/isotropic/output/" 
time_limit = "4:30:00"  
partition = "???" # change this to your partition
account = "???" # change this to your account 
nodes = [16,16,16,16]
