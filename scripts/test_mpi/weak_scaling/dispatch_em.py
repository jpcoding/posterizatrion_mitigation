...
mpirun -n {mpi_nps} $test --mpidims {" ".join(str(x) for x in mpi_dims )}  -m abs -e $eb --dir $data_dir  --outdir $out_dir --prefix vx --origdims {" ".join(str(x) for x in origdims )} --use_rbf 0 --local_edt 1 --local_quant 1
done
done 
